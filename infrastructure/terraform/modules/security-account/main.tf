...
resource "aws_securityhub_account" "security_account" { provider = aws.security }
...
resource "aws_s3_bucket_lifecycle_configuration" "cloudtrail" {
  bucket = aws_s3_bucket.cloudtrail.id
  rule {
    id = "7yr-retention"; status = "Enabled"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Sid = "DenyNonTLS"; Effect = "Deny"; Principal = "*"; Action = "s3:*"
        Resource = ["arn:aws:s3:::${aws_s3_bucket.cloudtrail.id}","arn:aws:s3:::${aws_s3_bucket.cloudtrail.id}/*"]
